...
```{r}
# Plotting method for aclrtm_accelerometery objects
plot(acc)
```
...
data(bench)
bench <- burst(bench, "accelerations_raw",
...
```{r}
# scratch file name defaults to "./tmp"
reservoir <- mine_reservoir(
  bench,           # can be a path to a .csv file with data in burst format
  overwrite = TRUE # should the scratch file be overwritten?
)
```
