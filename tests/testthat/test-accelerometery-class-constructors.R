test_that("numeric method builds a matrix with inferred axes", {
  x <- 1:12
  expect_message(out <- accelerometry(x), "inferred axes = \"xyz\"")
...
test_that("matrix method warns and overrides invalid existing column names", {
  ...
  expect_warning(out <- suppressMessages(accelerometry(x)),
...
test_that("matrix method warns when axes overrides existing column names", {
  ...
  expect_warning(accelerometry(x, axes = "zyx"), "overridden by")
})

test_that("matrix method errors on column count mismatch with axes", {
  x <- matrix(1:8, 4, 2)
  expect_error(accelerometry(x, axes = "xyz"), "but `x` has")
})

test_that("matrix method rejects matrices outside 1-3 columns", {
  x <- matrix(1:16, 4, 4)
  expect_error(accelerometry(x), "1, 2, or 3 columns")
})

# accelerometry.data.frame ----
...
test_that("data.frame method derives start_time and sampling_rate from ts_col", {
  ...
  expect_message(out <- accelerometry(df, ts_col = "timestamp"),
                 "estimated as 2 Hz")
...
test_that("data.frame method lets explicit sampling_rate override the estimate", {
  ...
  out <- accelerometry(df, sampling_rate = 99, ts_col = "timestamp")
...
test_that("data.frame method errors with >3 numeric columns and no axes", {
  ...
  expect_error(accelerometry(df), "at most 3")
})

test_that("data.frame method errors when ts_col is invalid", {
  df <- data.frame(x = 1:4, timestamp = 1:4)
  expect_error(accelerometry(df, ts_col = "nope"), "not found")
  expect_error(accelerometry(df, ts_col = 5), "out of range")
})

# Validation ----
test_that("validate_accelerometry passes through a valid object", {
  x <- new_accelerometry(matrix(1:6, 3, 2, dimnames = list(NULL, c("x","y"))))
  expect_identical(validate_accelerometry(x), x)
})

test_that("validate_accelerometry rejects wrong class", {
  x <- matrix(1:6, 3, 2)
  expect_error(validate_accelerometry(x), "aclrtm_accelerometry.*object")
})

test_that("validate_accelerometry rejects bad column counts", {
  x <- new_accelerometry(matrix(1:12, 3, 4))
  expect_error(validate_accelerometry(x), "1, 2, or 3 columns")
})

test_that("validate_accelerometry rejects invalid column names", {
  x <- new_accelerometry(matrix(1:6, 3, 2, dimnames = list(NULL, c("x","w"))))
  expect_error(validate_accelerometry(x), "subset of c\\('x','y','z'\\)")
})

test_that("validate_accelerometry rejects an invalid sampling_rate", {
  x <- new_accelerometry(matrix(1:6, 3, 2), sampling_rate = -1)
  expect_error(validate_accelerometry(x), "positive finite number")
})

test_that("validate_accelerometry rejects an invalid start_time", {
  x <- new_accelerometry(matrix(1:6, 3, 2), start_time = c(1, 2))
  expect_error(validate_accelerometry(x), "single POSIXct or positive numeric")
})
