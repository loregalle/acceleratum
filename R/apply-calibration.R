...
#'   around one or two axes (e.g. a device mounted upside down).
...
#'
#' @export
apply_cal <- function(A_sens, S, M, R, b) {
  UseMethod("apply_cal")
}

#' @export
#' @rdname apply_cal
apply_cal.matrix <- function(A_sens,
                             S = diag(ncol(A_sens)),
                             M = diag(ncol(A_sens)),
                             R = diag(ncol(A_sens)),
                             b = rep(0, ncol(A_sens))) {
...
#' @export
#' @rdname apply_cal
apply_cal.aclrtm_burst <- function(A_sens,
                                   S = diag(nchar(attr(A_sens, "axes"))),
                                   M = diag(nchar(attr(A_sens, "axes"))),
  ...
  data_col <- attr(A_sens, "data_col")
  end_burst <- cumsum(sapply(A_sens[[data_col]], nrow))
  start_burst <- c(1, end_burst[1:(length(end_burst)-1)]+1)
  mat <- do.call(rbind, A_sens[[data_col]])
  cn <- colnames(mat)
  mat <- sweep(mat, 2L, b, "-") %*% solve(S) %*% t(solve(M)) %*% R
  colnames(mat) <- cn
  for (i in seq_len(nrow(A_sens))) {
    A_sens[[data_col]][[i]] <- mat[start_burst[i]:end_burst[i],]
  }
